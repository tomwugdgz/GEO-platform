...
def _kill_tree(pid: int) -> bool:
    """跨平台终止进程（POSIX 杀进程组，Windows 用 taskkill /T 杀进程树）。

    上游只用 `os.killpg(os.getpgid(pid))`，这两个函数在 Windows 上都不存在，
    会让「停止任务」按钮直接抛 AttributeError。这里做平台分支：

    - POSIX：优先杀整个进程组（子进程会连带被回收，避免孤儿）
    - Windows：用 `taskkill /PID <pid> /T /F` 杀进程树；taskkill 不可用时
      退回 `proc.terminate()` / `os.kill(pid, SIGTERM)`
    """
    if os.name == "nt":
        try:
            import subprocess
...
def _is_alive(pid: int) -> bool:
    ...
    上游用 `os.kill(pid, 0)` 探活 —— POSIX 上这是标准做法，但 Windows 的
    `os.kill` 只支持 CTRL_C_EVENT / CTRL_BREAK_EVENT 等有限信号，
    发 0 会抛 `OSError`，导致孤儿回收逻辑失灵（残留 running 记录会永久
    挡住同项目的新任务）。
    """
    if os.name == "nt":
        # Windows：用 OpenProcess 探测（需要 tasklist/psutil 之外的零依赖方案）
        try:
            import ctypes

...
# ── Windows 兼容层 ───────────────────────────────────────────────
# 上游只为 macOS / Linux 编写，`fcntl` 是 POSIX 专有模块，Windows CPython
# 不提供，直接 import 会导致任何命令都以 ModuleNotFoundError 退出。
# 这里在缺失时降级到内置的 msvcrt 文件锁实现（接口完全一致）。
try:
    import fcntl  # type: ignore[import-not-found]
except ModuleNotFoundError:  # Windows
    try:
        from _win_fcntl import fcntl_shim as fcntl  # type: ignore[assignment]
    except ModuleNotFoundError:
        # 脚本目录不在 sys.path 时（被作为包导入）再兜一次
        sys.path.insert(0, str(__import__("pathlib").Path(__file__).resolve().parent))
        from _win_fcntl import fcntl_shim as fcntl  # type: ignore[assignment]

# ── Windows 控制台编码兜底 ────────────────────────────────────────
# 中文 Windows 控制台默认 GBK，打印中文/emoji 会 UnicodeEncodeError 中断管线。
...
# 移植到 FastAPI 项目后，geolib.py 位于 backend/geolook/，
# 上游的 ROOT 指向 scripts/ 的父目录（geolook 根）。
# 这里改为 backend/geolook/ 本身，让 work/ 数据落在 backend/geolook/work/ 下，
# 与 FastAPI 的 venv/site-packages 隔离。

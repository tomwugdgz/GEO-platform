...
def _harden_env_permissions(path):
    """收紧 .env 权限，避免同机其他用户读取 API 密钥。

    - POSIX：chmod 600（上游行为）
    - Windows：chmod 只影响只读位，POSIX 权限位无意义；改用 `icacls` 移除
      Everyone / Users 的继承权限，只保留当前用户。icacls 不可用时静默跳过
      —— 密钥仍在本地文件里，不会外泄，只是少了这层加固。
    """
    if os.name == "nt":
        try:
            import subprocess
            ...
            if not user:
                return
            # /inheritance:r 去掉继承，再只授予当前用户完全控制
            subprocess.run(
